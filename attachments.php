...
<?php
	$fullname='';
		...
		global $employeedb;
		//check if already have profile picture
		$profile=$employeedb->query("select * from images where empId=".$_GET['id']);
		if($profile->num_rows>0){
		echo ("<SCRIPT LANGUAGE='JavaScript'>
        
        window.location.href='editattachments.php?id=". $_GET['id'] ."';
        </SCRIPT>");
		}
		...
		if($result->num_rows <=0){
			...
			echo ("<SCRIPT LANGUAGE='JavaScript'>
        window.alert('Unknown Employee');
        window.location.href='home.php';
        </SCRIPT>");
		}else{
			$emp= $result->fetch_object();
			$result->close();
			$fullname='Profile picture for '.$emp->lname.','.$emp->fname .' '. $emp->mname;	
		}
        if(count($_FILES) > 0) {
			if(is_uploaded_file($_FILES['userImage']['tmp_name'])) {
						$imgData =addslashes(file_get_contents($_FILES['userImage']['tmp_name']));
						$imageProperties = getimageSize($_FILES['userImage']['tmp_name']);
						$sql = "INSERT INTO images(empId,img,file_name)
						VALUES('{$_GET['id']}', '{$imgData}','{$imageProperties['mime']}')";
